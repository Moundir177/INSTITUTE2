...
export function TestimonialCarousel({
  ...
  }, [currentIndex, autoPlay, interval, isPaused, testimonials.length]);
  ...
  const bgColor = variant === "light" ? "bg-white" : "bg-ukblue";
  const textColor = variant === "light" ? "text-gray-800" : "text-white";
  const quoteColor = variant === "light" ? "text-ukblue/10" : "text-white/10";
  const buttonBgColor = variant === "light" ? "bg-silver-100 hover:bg-silver-200" : "bg-white/10 hover:bg-white/20";
  const buttonTextColor = variant === "light" ? "text-gray-600" : "text-white";
  const secondaryTextColor = variant === "light" ? "text-gray-500" : "text-silver-300";
            ...
            >
              ...
              <div className="flex items-center justify-center">
                ...
                <div className="ml-4 text-left">
                  <h4 className={`font-semibold ${textColor}`}>{testimonials[currentIndex].name}</h4>
                  {testimonials[currentIndex].role && (
                    <p className={secondaryTextColor}>{testimonials[currentIndex].role}</p>
                  )}
                  {testimonials[currentIndex].company && (
                    <p className="text-gold-500 text-sm">{testimonials[currentIndex].company}</p>
                  )}
                </div>
              </div>
            </motion.div>
